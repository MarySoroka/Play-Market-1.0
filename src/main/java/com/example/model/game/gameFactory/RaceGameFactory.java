...
public class RaceGameFactory implements GameFactory {
    @Override
    public Game createGame(String[] data) {
        try {
            return new RaceGame(data[0],data[1],new SimpleDateFormat("dd/mm/yyyy").parse(data[2]),
                    new Graphics(GraphicsMode.getByName(data[3]))
                    , OnlineStatus.getInstance(data[4]), Integer.parseInt(data[5]));
